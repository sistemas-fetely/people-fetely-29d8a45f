...
export function BeneficiosCLTView({ canManage, isAdmin }: Props) {
  ...
  const colaboradoresComBeneficio = new Set(beneficios.filter((b) => b.status === "ativo").map((b) => b.colaborador_id)).size;

  const resetForm = () => {
    setFormColaboradorId(""); setFormTipo("vt"); setFormDescricao(""); setFormOperadora("");
  ...
  const openEdit = (b: BeneficioComColaborador) => {
    setEditing(b); setFormColaboradorId(b.colaborador_id); setFormTipo(b.tipo);
  ...
  const handleSubmit = () => {
    if (editing) {
      ...
      criarMut.mutate({
        colaborador_id: formColaboradorId, tipo: formTipo,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input placeholder="Buscar colaborador ou operadora..." value={busca} onChange={(e) => setBusca(e.target.value)} className="pl-9" />
        </div>
      ...
      <Dialog open={showForm} onOpenChange={(o) => { if (!o) { setShowForm(false); resetForm(); } else setShowForm(true); }}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editing ? "Editar Benefício" : "Novo Benefício CLT"}</DialogTitle></DialogHeader>
            ...
            {!editing && (
              <div className="space-y-1">
                ...
                <Select value={formColaboradorId} onValueChange={setFormColaboradorId}>
                  ...
                  <SelectContent>
                    {colaboradores.map((c) => (<SelectItem key={c.id} value={c.id}>{c.nome_completo} — {c.departamento}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={!editing && !formColaboradorId}>{editing ? "Salvar" : "Cadastrar"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
