-- ============================================================================
-- Fix: vw_contas_pagar_consolidado — remover referências a aguardando_pagamento
-- ============================================================================
-- status_efetivo e atrasada ainda referenciavam status legado pré-Doutrina #95.
-- Doutrina #95: status aguardando_pagamento substituído por enviado_para_pagamento.
-- ============================================================================
...
SELECT
  ...
  CASE
    WHEN cp.status = 'enviado_para_pagamento'::text AND mb.conciliado = true THEN 'pago_conciliado'::text
    WHEN cp.status = 'enviado_para_pagamento'::text AND mb.id IS NOT NULL THEN 'em_movimentacao'::text
    ELSE cp.status
  END AS status_efetivo,
  ('doc_pendente'::text IN (SELECT jsonb_array_elements_text(cp.tags))) AS tem_doc_pendente,
  cp.data_vencimento < CURRENT_DATE
    AND (cp.status = ANY (ARRAY['aberto'::text, 'aprovado'::text, 'enviado_para_pagamento'::text, 'doc_pendente'::text]))
